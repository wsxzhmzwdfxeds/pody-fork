...
func ViewsTotal(g *gocui.Gui, lMaxX, lMaxY int) error {
	viewDebug(g, lMaxX, lMaxY)
	viewLogs(g, lMaxX, lMaxY)
	viewNamespaces(g, lMaxX, lMaxY)
	viewOverlay(g, lMaxX, lMaxY)
	viewTitle(g, lMaxX, lMaxY)
	viewPods(g, lMaxX, lMaxY)
	viewStatusBar(g, lMaxX, lMaxY)
	return nil
}
// View: Overlay
...
func viewPodsRefreshList(g *gocui.Gui) {
	g.Update(func(g *gocui.Gui) error {
		...
		viewPodsAddLine(v, lMaxX, "NAME", "CPU", "MEMORY", "READY", "STATUS", "RESTARTS", "AGE")
		...
		if len(pods.Items) > 0 {
			...
			for _, pod := range pods.Items {
				n := pod.GetName()
				c,_ := columnHelperResource(pod.Spec.Containers)
				_,m := columnHelperResource(pod.Spec.Containers)
				...
				viewPodsAddLine(v, lMaxX, n, c, m, cr, s, r, a)
...
func viewPodsAddLine(v *gocui.View, maxX int, name, cpu, memory, ready, status, restarts, age string) {
	wN := maxX - 43 // 54 // TODO CPU + Memory #20
	if wN < 45 {
		wN = 45
	}
	line := pad.Right(name, wN, " ") +
		pad.Right(cpu, 10, " ") + // TODO CPU + Memory #20
		pad.Right(memory, 15, " ") + // TODO CPU + Memory #20
		pad.Right(ready, 10, " ") +
		pad.Right(status, 10, " ") +
